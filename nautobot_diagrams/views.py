...
PLUGIN_SETTINGS = settings.PLUGINS_CONFIG.get("nautobot_diagrams", dict())
...
def get_vlan_topology(nb_devices_qs, vlans):
    ...
    for device in devices:
        ...
        topology_dict['nodes'].append({
            # 'id': device.id, # original from netbox_ui_plugin
            'id': str(device.id),
            'name': device.name,
            ...
            if len(interface_trace) != 0:
                ...
                if (mapping_link not in mapped_links) and (mapping_link.reverse() not in mapped_links):
                    ...
                    topology_dict['links'].append({
                        # 'id': source_cable[1].id,  # original from netbox_ui_plugin
                        'id': str(source_cable[1].id),
                        # 'source': source_cable[0].device.id, # original from netbox_ui_plugin
                        'source': str(source_cable[0].device.id),
                        # 'target': dest_cable[-1].device.id, # original from netbox_ui_plugin
                        'target': str(dest_cable[-1].device.id),
...
def get_topology(nb_devices_qs):
    ...
    for nb_device in nb_devices_qs:
        ...
        if links_to_device:
            for link in links_to_device:
                ...
                device_is_passive = True
        topology_dict['nodes'].append({
            # 'id': nb_device.id, # original from netbox_ui_plugin
            'id': str(nb_device.id),
            'name': nb_device.name,
            'dcimDeviceLink': device_url,
            'primaryIP': primary_ip,
    ...
    for link in links:
        ...
        topology_dict['links'].append({
            # 'id': link.id, # original from netbox_ui_plugin
            'id': str(link.id),
            # 'source': link.termination_a.device.id, # original from netbox_ui_plugin
            'source': str(link.termination_a.device.id),
            # 'target': link.termination_b.device.id, # original from netbox_ui_plugin
            'target': str(link.termination_b.device.id),
        ...
        # if NETBOX_CURRENT_VERSION >= version.parse("2.10.1"):
            ...
            # This is from the original src code and refers to Netbox Version 2.10.1.
...
class TopologyView(PermissionRequiredMixin, View):
    ...
    if NAUTOBOT_CURRENT_VERSION >= version.parse("1.2"):
        template_name = 'nautobot_diagrams/topology.html'
    # else:
    #     template_name = 'nautobot_diagrams/topology_3.x.html'
    #     # NZE: kept them from the original source just in case
    #     # any compatibility issues or behavior changes are encountered due to different versions.
...
class SiteTopologyView(TopologyView):
    # if NETBOX_CURRENT_VERSION >= version.parse("3.0"):
    if NAUTOBOT_CURRENT_VERSION >= version.parse("1.2"):
        template_name = 'nautobot_diagrams/site_topology.html'
    # else:
    #     # NZE: kept them from the original source just in case
    #     # any compatibility issues or behavior changes are encountered due to different versions.
    #     template_name = 'nautobot_diagrams/site_topology_3.x.html'